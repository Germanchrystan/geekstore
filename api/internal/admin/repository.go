...
func (r *repository) GetAllUsers(ctx context.Context) ([]domain.User, error) { // ADD PAGINATION
	query := "SELECT id, username,firstname, lastname, email, is_active, is_admin, is_banned FROM users;"
...
func (r *repository) PostProduct(ctx context.Context, product domain.Product) (domain.Product, error) {
	productQuery := "INSERT into products(\"product_name\", \"price\",\"product_description\", \"subgenre_id\", \"category_id\") VALUES ($1, $2, $3, $4, $5, $6);"
	...
	_, err = stmt.Exec(product.Name, product.Price, product.Description, product.SubgenreId, product.CategoryId)
	if err != nil {
		return domain.Product{}, errors.New("Unable to create product")
	}
...
func (r *repository) DeleteProduct(ctx context.Context, product_id string) error {
	productQuery := "DELETE FROM products WHERE id=$1;"
	stmt, err := r.db.Prepare(productQuery)
	if err != nil {
...
func (r *repository) UpdateProduct(ctx context.Context, product domain.Product) (domain.Product, error) {
	productQuery := "UPDATE products SET product_name=$1, price=$2, product_description=$3, subgenre_id=$4, category_id=$5 WHERE id=$6;"
	stmt, err := r.db.Prepare(productQuery)
	if err != nil {
		return domain.Product{}, errors.New("Product could not be updated")
	}

	res, err := stmt.Exec(product.Name, product.Price, product.Description, product.SubgenreId, product.CategoryId, product.Id)
	if err != nil {
		return domain.Product{}, errors.New("Product could not be updated")
	}
	affected, err := res.RowsAffected()
...
func (r *repository) ToggleUserBan(ctx context.Context, user_id string) error {
	var is_banned bool
	selectQuery := "SELECT is_banned FROM users WHERE id=$1;"
...
func (r *repository) ToggleUserAdmin(ctx context.Context, user_id string) error {
	var is_admin bool
	selectQuery := "SELECT is_admin FROM users WHERE id=$1;"
	row := r.db.QueryRow(selectQuery, user_id)
	_ = row.Scan(&is_admin)
...
func (r *repository) GetProductById(ctx context.Context, productID string) (domain.Product, error) {
	...
	productQuery := "SELECT id, product_name, price, product_description, subgenre_id, category_id FROM products WHERE _id=$1;"

	row := r.db.QueryRow(productQuery, productID)
	err := row.Scan(&product.Id, &product.Name, &product.Price, &product.Description, &product.SubgenreId, &product.CategoryId)
	if err != nil {
